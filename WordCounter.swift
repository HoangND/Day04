...
func < (word1: Word, word2: Word) -> Bool {
...
//        self.countWord("aa nn mm cc kk ee cc bb bb ee dd aa")
        ...
        println("\(WordCountNSort.count)")
//        println("\(WordCountNSort[0].word)")
    }
    ...
    func bubbleSort(inout wordArray: [Word]){
        for (var i=0; i<wordArray.count-1; i++) {
            for (var j=0; j<=wordArray.count-2; j++) {
                if (wordArray[j] > wordArray[j+1]) {
                    let temp1 = wordArray[j]
